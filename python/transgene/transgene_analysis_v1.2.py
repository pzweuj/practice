...
# v1.2 部分连接位点直接有两边均比对不上的碱基，修复这部分的内容，调整结果呈现方式
#####
import re
import bamnostic
import time
...
for read in bamFile:
	...
	## 解析过滤后的reads
		...
		# ## 宿主序列与质粒序列的合成方式
		# hostCigarDirect = cigar.split("M")
		# insertCigarDirect = tagSA[3]
		# if hostCigarDirect[1] == "":
		# 	hostMapAt = "right"
		# 	hostSoft = hostCigarDirect[0].split("S")[0]
		# 	hostMap = hostCigarDirect[0].split("S")[1]
		# 	insertSoft = insertCigarDirect.split("M")[1].split("S")[0]
		# 	insertMap = insertCigarDirect.split("M")[0]
		# else:
		# 	hostMapAt = "left"
		# 	hostSoft = hostCigarDirect[0]
		# 	hostMap = hostCigarDirect[1].split("S")[0]
		# 	insertSoft = insertCigarDirect.split("S")[0]
		# 	insertMap = insertCigarDirect.split("S")[1].split("M")[0]
