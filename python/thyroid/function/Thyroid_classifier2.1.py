...
## 主方法
def main(arff, modelFile, resultsFile, method, n_est, depth, classifier, V600E, Tree, ROC):
...
if __name__ == "__main__":
	...
	parser.add_argument("-tree", "--tree", type=bool,
		help="随机输出一个决策树，默认不输出", default=False)
	parser.add_argument("-ROC", "--ROC", type=bool,
		help="输出ROC曲线，默认不输出", default=False)
	if len(sys.argv[1:]) == 0:
		parser.print_help()
		parser.exit()
	args = parser.parse_args()
	main(arff=args.arff, modelFile=args.model, resultsFile=args.output, method=args.method, n_est=args.n_est, depth=args.depth, classifier=args.classifier, V600E=args.fix, Tree=args.tree, ROC=args.ROC)
