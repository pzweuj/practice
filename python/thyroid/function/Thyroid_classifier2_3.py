...
## 主方法
def main(arff, modelFile, resultsFile, method, n_est, depth, V600E, Tree, ROC, feature):
	...
	if method == "train":
		...
		if n_est == 0 and depth == 0:
			...
			for nn in tryDict["n_estimators"]:
				for mm in tryDict["max_depth"]:
					model = RandomForestClassifier(n_estimators=nn,
						bootstrap=True, max_depth=mm, max_features=max_f)
					scores_check = cross_val_score(model, xtrain, ytrain, cv=10, scoring="accuracy")
					if scores_check.mean() > scores_out:
						scores_out = scores_check.mean()
						n_max = nn
						m_max = mm
			print("树数设置\t" + str(n_max))
			print("深度\t" + str(m_max))
			print("\n")
			model = RandomForestClassifier(n_estimators=n_max,
				bootstrap=True, max_depth=m_max, max_features=max_f)
...
if __name__ == "__main__":
	...
	parser.add_argument("-n", "--n_est", type=int,
		help="可选，设置树数，仅在训练模式下有效，默认为自动适应", default=0)
	parser.add_argument("-d", "--depth", type=int,
		help="可选，设置决策树深度，仅在训练模式下有效，默认为自动适应", default=0)
	...
	main(arff=args.arff, modelFile=args.model, resultsFile=args.output, method=args.method, n_est=args.n_est, depth=args.depth, V600E=args.fix, Tree=args.tree, ROC=args.ROC, feature=args.feature)
