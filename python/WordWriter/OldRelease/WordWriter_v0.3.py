...
def fillTable(document, tag, insertTable):
	...
	while row_id <= rowToFill:
		for co in range(columnToFill):
			table.cell(row_id, co).text = tableToFill.iloc[start, co]
...
## 参考文献
def insertReference(document, tag, referenceFile):
	# ref = open(referenceFile, "r")
...
# fillTable(docxFile, "#[TABLE-1]#", "testtable.txt")
